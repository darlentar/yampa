...
def base64_to_audio(base64_str:str):
    sound = AudioSegment(data=base64.b64decode(base64_str), sample_width=2, frame_rate=24_000, channels=1)
    play(sound)
...
class EventProcessors:
    ...
    def conversation_item_created(self, event):
        ...
        if newItem['id'] in self.queuedSpeechItems:
            newItem['formatted']['audio'] = self.queuedSpeechItems[newItem['id']]['audio']
            del self.queuedSpeechItems[newItem['id']]

        # Population du texte si présent
        if 'content' in newItem:
            textContent = [c for c in newItem['content'] if c['type'] in ['text', 'input_text']]
        ...
        if newItem['id'] in self.queuedTranscriptItems:
            newItem['formatted']['transcript'] = self.queuedTranscriptItems[newItem['id']]['transcript']
            del self.queuedTranscriptItems[newItem['id']]

        if newItem['type'] == 'message':
            newItem['status'] = 'completed' if newItem['role'] == 'user' else 'in_progress'
                    ...
                    for item in processor.items:
                        if formatted := item.get("formatted"):
                            ...
                            if transcript := formatted.get("transcript"):
                                print(transcript)
                case "response.output_item.done" | "response.content_part.done" | "session.created" | "response.audio_transcript.done" | "response.audio.done" :
